...
static char gaussian_path[PATH_MAX];
...
int main(void){
  ...
  sprintf(gaussian_path, "%s/.codes/1d_gaussian.py", path_tmp);
  
  printf("PREPROCESSOR IS READY\n");
  
  dir_no=START_1/DIR_MAX+1;
  ...
  while(1){
    ...
    if(pid_g==0){
      execl(gaussian_path, gaussian_path, path_tmp, NULL);
      perror("execl_preprocessor");
      exit(1);
    }
    waitpid(pid_g, &status, 0);
